...
브라우저에서 한 번에 확인하려면 Django 테스트 페이지 `http://localhost:8200/api-test/` 를 사용한다. 이 페이지는 내부 토큰을 자동으로 헤더에 붙이므로 별도 입력 없이 상태 조회, 공고 조회, 상담 정규화, GPT/job parser 테스트, 매칭, 수동 크롤링 큐잉 API를 직접 호출할 수 있다. 상세 사용법은 `docs/API_TEST_PAGE.md`를 참고한다.
